...
def docquery_feature(user_question, file_path=r'C:\Users\Admin\Downloads\Dubvoice Abstract.pdf'):
    p = pipeline('document-question-answering')
    st.write(os.getcwd(), file_path)
    
    if os.path.exists(file_path):
        doc = document.load_document(file_path)
        
        st.write("User Question:", user_question)
        st.write("Document Path:", file_path)
        
        # Perform document question answering
        result = p(question=user_question, **doc.context)
        ...
        st.write("File not found. Please make sure the file exists.")
...
def question_from_pdf():
    ...
    if uploaded_file is not None:
        ...
        docquery_feature(str(uploaded_file.name))
    else:
